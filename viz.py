...
json_file = 'dataset/evaluation/evaluation_metrics.json'
...
total_seqs = [item['total_sequences'] for item in data['accuracy_by_level']]
...
misclass = data['misclassifications_genus']['top_patterns']
...
source_names = []
source_counts = []
for src in data['ground_truth_sources']:
    name = Path(src).stem.replace('_accession_full_lineage', '')
    source_names.append(name)

source_counts = [65678, 21278, 7951, 6574]  # ITS, 16S, SSU, LSU
...
plt.tight_layout()
output_file = output_dir / 'accuracy_cascade.png'
...
width = 0.35
bars1 = ax.bar(x - width/2, correct_seqs, width, label='Correct', color='#06A77D')
bars2 = ax.bar(x + width/2, incorrect_seqs, width, label='Incorrect', color='#D5573B')
ax.set_xlabel('Taxonomic Level', fontsize=12, fontweight='bold')
...
plt.tight_layout()
output_file = output_dir / 'correct_vs_incorrect.png'
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"Saved: {output_file}")
plt.close()
...
plt.tight_layout()
output_file = output_dir / 'novelty_confusion_matrix.png'
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"Saved: {output_file}")
plt.close()
...
bars = ax.barh(novelty_metrics, novelty_values, color=colors, edgecolor='black', linewidth=1.5)
...
plt.tight_layout()
output_file = output_dir / 'novelty_metrics.png'
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"Saved: {output_file}")
plt.close()
...
plt.tight_layout()
output_file = output_dir / 'cluster_purity.png'
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"Saved: {output_file}")
plt.close()
...
ax.set_title('Top 10 Misclassification Patterns (Genus Level)', fontsize=14, fontweight='bold')
ax.grid(True, alpha=0.3, axis='x')
for i, (bar, count) in enumerate(zip(bars, misclass_counts)):
    ax.text(count + 0.5, i, str(count), va='center', fontsize=10, fontweight='bold')
plt.tight_layout()
output_file = output_dir / 'top_misclassifications.png'
plt.savefig(output_file, dpi=300, bbox_inches='tight')
print(f"Saved: {output_file}")
plt.close()
...
ax4.barh(novelty_metrics, novelty_values, color=colors)
ax4.set_xlim(0, 1)
...
genus_acc = accuracies[taxonomic_levels.index('Genus')]
species_acc = accuracies[taxonomic_levels.index('Species')]
mean_purity = cp['mean_purity']
f1 = nov['f1_score']
...
for i, stat in enumerate(key_stats):
    color = '#E63946' if 'POOR' in stat else '#2E86AB'
    weight = 'bold' if 'POOR' in stat or 'Rating' in stat else 'normal'
    ax6.text(0.1, y_start - i*0.11, stat, fontsize=11, fontweight=weight, 
...
print("  - accuracy_cascade.png")
print("  - correct_vs_incorrect.png")
print("  - novelty_confusion_matrix.png")
print("  - novelty_metrics.png")
